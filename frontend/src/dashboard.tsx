...
export default function Dashboard({ usuario }: { usuario: any }) {
    ...
    // 2. Cargar el JSON nativo desde Db2 cuando se selecciona un presupuesto
    useEffect(() => {
    ...
    if (!datosJSON) {
        return (
            <div style={{ padding: '40px', textAlign: 'center', background: 'white', borderRadius: '10px' }}>
                <h3 style={{ color: '#666' }}>Cargando datos del dashboard...</h3>
            </div>
        );
    }

    // 3. Procesamiento de Datos: Agrupar los detalles por Categoría para el gráfico de pastel
    const datosPastel = datosJSON.detalles ? datosJSON.detalles.reduce((acc: any[], curr: any) => {
        const existente = acc.find(item => item.name === curr.categoria);
        if (existente) {
    ...
    return (
        <div style={{ background: '#f8f9fa', padding: '20px', borderRadius: '10px' }}>
            ...
            <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
                ...
                {/* Tabla Rápida de Detalles */}
                <div style={{ flex: 1, minWidth: '400px', background: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', maxHeight: '400px', overflowY: 'auto' }}>
